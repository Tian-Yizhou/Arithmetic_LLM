"""Distributed model evaluation module for arithmetic LLM.

This module provides distributed evaluation capabilities for trained arithmetic models
using HuggingFace Accelerate, enabling multi-GPU evaluation with automatic data sharding
and metric aggregation across processes.
"""
...
class ModelEvaluator:
    """Distributed evaluator for trained arithmetic models.
    ...
    Supports multi-GPU distributed evaluation via HuggingFace Accelerate.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            base_checkpoint_path: Path to base model checkpoint when evaluating a LoRA adapter
            accelerator: Optional Accelerator instance for distributed evaluation
            device: Device for inference ('cuda', 'mps', or 'cpu'). Ignored if accelerator is provided.
        """
        self.accelerator = accelerator
        self.device = accelerator.device if accelerator else device
        self.model_path = model_path
        self.tokenizer_path = tokenizer_path
        self.base_checkpoint_path = base_checkpoint_path
        self.is_distributed = accelerator is not None
        self.is_main = (not self.is_distributed) or accelerator.is_local_main_process
    ...
    def evaluate(
        ...
        When an accelerator is provided, test samples are sharded across processes
        and metrics are aggregated via accelerator.reduce().
        ...
        # Generate test set with fixed seed for reproducibility across all processes
        ...
        random.setstate(rng_state)  # Restore RNG state

        if self.is_main:
            print(f"Generated {len(test_expressions)} valid test expressions")

        # Shard test set across processes for distributed evaluation
        if self.is_distributed:
            num_processes = self.accelerator.num_processes
            process_index = self.accelerator.process_index

            # Divide samples across processes (handle remainder)
            total_samples = len(test_expressions)
            per_process = total_samples // num_processes
            remainder = total_samples % num_processes

            if process_index < remainder:
                start = process_index * (per_process + 1)
                end = start + per_process + 1
            else:
                start = remainder * (per_process + 1) + (process_index - remainder) * per_process
                end = start + per_process

            local_expressions = test_expressions[start:end]
            local_answers = test_answers[start:end]
        else:
            local_expressions = test_expressions
            local_answers = test_answers
            start = 0

        # Evaluate model on local shard using batches
        correct = 0
        parseable = 0
        total_length = 0
        sample_outputs = []
        ...
        if self.is_main:
            ...
            if self.is_distributed:
                print(f"  Distributed across {self.accelerator.num_processes} processes")
                print(f"  This process evaluates {len(local_expressions)} samples")

        # Process in batches
        for batch_start in range(0, len(local_expressions), batch_size):
            batch_end = min(batch_start + batch_size, len(local_expressions))
            batch_expressions = local_expressions[batch_start:batch_end]
            batch_answers = local_answers[batch_start:batch_end]
            ...
            for i, (expression, ground_truth, generated_text) in enumerate(
                ...
                # Save sample outputs (first 10 global indices, main process only)
                global_idx = start + batch_start + i
                if self.is_main and global_idx < 10:
            ...
            # Progress update
            if self.is_main:
                print(f"Evaluated {batch_end}/{len(local_expressions)} samples (this process)")

        # Aggregate metrics across processes
        if self.is_distributed:
            correct_tensor = torch.tensor([correct], dtype=torch.float, device=self.accelerator.device)
            parseable_tensor = torch.tensor([parseable], dtype=torch.float, device=self.accelerator.device)
            total_length_tensor = torch.tensor([total_length], dtype=torch.float, device=self.accelerator.device)
            local_count_tensor = torch.tensor([len(local_expressions)], dtype=torch.float, device=self.accelerator.device)

            self.accelerator.wait_for_everyone()

            correct = int(self.accelerator.reduce(correct_tensor, reduction="sum").item())
            parseable = int(self.accelerator.reduce(parseable_tensor, reduction="sum").item())
            total_length = int(self.accelerator.reduce(total_length_tensor, reduction="sum").item())
            total = int(self.accelerator.reduce(local_count_tensor, reduction="sum").item())
        else:
        ...
        # Save results if output directory is provided (main process only)
        if output_dir and self.is_main:
    ...
    def _save_results(
        ...
        with open(summary_path, 'w') as f:
            f.write("=" * 60 + "\n")
            f.write("ARITHMETIC LLM EVALUATION SUMMARY (DISTRIBUTED)\n")
            f.write("=" * 60 + "\n\n")
            f.write(f"Model: {self.model_path}\n")
            f.write(f"Tokenizer: {self.tokenizer_path}\n")
            if self.is_distributed:
                f.write(f"Number of processes: {self.accelerator.num_processes}\n")
